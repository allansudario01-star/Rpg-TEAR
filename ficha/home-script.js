...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loginBtn.addEventListener('click', async () => {
        // Usar o mesmo showAuthDialog do seu script.js
        const authResult = await showAuthDialog();
        if (authResult) {
            // Recarregar personagens após login
...
createNewBtn.addEventListener('click', () => {
    // Criar um link que FORÇA o recarregamento da página
    const url = new URL('ficha.html', window.location.href);
    url.searchParams.append('new', 'true'); // Adiciona ?new=true na URL
    url.searchParams.append('t', Date.now()); // Adiciona timestamp para evitar cache
    ...
    function loadCharacter(characterId) {
        // Salva no localStorage o ID do personagem a ser carregado
        localStorage.setItem('currentCharacterId', characterId);
        // Redireciona para a página da ficha
        window.location.href = 'ficha.html';
    }
    
    // Reutilizar sua função showAuthDialog (copiar do script.js)
